...
namespace Nodes.Sandbox
{
    public class SandboxNodelet
    {
        ...
        private void TestLoadBalancingRouter (Options options)
        {
             ...
             * waiting for work if the router goes down and the router will hang if the server goes down. If 
             * the static nodes die the workers must be restarted. Maybe some kind of time out will 
             * solve this problem? Maybe we can let the process die (or restart) if no responses are coming back?
             * 
             * This is also got me thinking about the message reliability. Assuming the server was reading from a queue
